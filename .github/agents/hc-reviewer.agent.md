...
The CRG structural skills (`/hc-review-delta`, `/hc-review-pr`) are inputs to the evidence assessment — they surface blast radius and test gaps — but they are not the sole evidence. Implementer output, focused test results, and decisions log entries are equally valid evidence.
...
- For incremental reviews: invoke `/hc-review-delta`
- For PR reviews: invoke `/hc-review-pr`
...
- Spawn @hc-explorer for additional context if needed — be specific about what you need; read the written bundle path, not inline content
...
- If CRG skills require `mode: mcp`, verify the mode before invoking `/hc-review-delta` or `/hc-review-pr`; in `mcp` mode, CRG failure is a setup gap, while `off` mode is an explicit manual-review fallback
