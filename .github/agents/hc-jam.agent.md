...
- **Delegate codebase exploration to sub-agents** — spawn @hc-explorer with a specific question; read only the written bundle path, not inline content
...
5. If needed, spawn @hc-explorer to gather codebase context — explorer is workspace-aware and writes context bundles to disk for downstream agents
...
**Preferred pattern:** Run this phase at the top level of the main CLI chat (not via `@hc-helix`). The host agent has the `ask_user` tool and can conduct the dialogue natively with structured form inputs — no relay, no wasted tokens.
...
- If the user wants to explore the codebase to inform the intent, spawn @hc-explorer subagent
- Keep the session focused — if scope creep happens, call it out
- The refined intent should be specific enough that a hc-planner agent can produce a PRD without ambiguity
