...
Produce `workspaces/{workspace-name}/task-boards/{feature-name}.md` by invoking the `/hc-task-board` skill with the **Create** operation.

The `/hc-task-board` skill defines the canonical board format (header, dependency graph, sections, per-task structure). Follow its format exactly — do not invent an alternative layout.
