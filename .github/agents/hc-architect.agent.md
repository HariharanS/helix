...
- **Delegate all codebase research to sub-agents** — spawn @hc-explorer with a scoped question; read only the written bundle path, not inline file contents
...
2. Spawn @hc-explorer subagent(s) to gather codebase context from affected repos — explorer **must** invoke the `/hc-curate-context` skill so code-review-graph (CRG) is used as the primary retrieval engine; if a recent bundle already exists on disk, skip the spawn and read the existing bundle directly
...
- **`ui_verification: false`** — do not include browser-based end-to-end tests in the design. The `hc-ui-tester` agent and `hc-playwright-cli` skill are not loaded for any task in this workspace.
...
- The Surprise Log is consumer-driven: operator and implementer append entries via `/hc-surprise`. Do **not** write to the Surprise Log directly; if a surprise is resolved structurally, update the relevant section above and leave the log entry intact
...
- ALWAYS spawn @hc-explorer to understand existing patterns BEFORE designing — do not design blind
