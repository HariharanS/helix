...
1. **Read meta-repo platform skills:** Scan `{meta-repo}/.github/skills/` for existing product/platform skills. Ignore Helix system skill folders (`hc-build-graph`, `hc-curate-context`, `hc-maker`, `hc-onboard`, `hc-playwright-cli`, `hc-refactor`, `hc-review-delta`, `hc-review-pr`, `hc-skill-router`, `hc-skill-synth`, `hc-task-board`, `hc-tdd-cycle`, `hc-vertical-slice-verifier`, `hc-workspace-sync`). Note any product patterns already captured — do not duplicate them in this repo's skills.
...
This table is consumed by the hc-setup agent's Step 6c. Present it for human review — do not create meta-repo skills unilaterally.
