...
│   ├── hooks/
│   │   └── logs/
│   ├── instructions/           ← generated by setup/onboarding; gitignored summaries
│   ├── prompts/
│   ├── skills/
...
Generated by setup/onboarding (gitignored, not managed by `helix-core` sync):

- `instructions/` — workspace- and repo-scoped instruction summary files emitted during setup and onboarding
...
7. scan readiness, generate `<workspace-id>.code-workspace` at the meta-repo root, and refresh `.github/instructions/*.instructions.md`
