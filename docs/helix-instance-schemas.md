...
Purpose: route agents from a repo/path/task to the right Helix skill guidance without relying on host-specific nested skill loading.

Generated by: `install-helix.ps1` for core `hc-*` skill entries and `setup-workspace.ps1` for active workspace repo-local candidates.

Key fields:

| Field | Required | Type | Notes |
|---|---:|---|---|
| `schema_version` | yes | number | Current value: `1` |
| `updated_at` | yes | string | ISO timestamp |
| `active_workspace` | no | string/null | Workspace used when repo candidates were scanned |
| `skills` | yes | list | Core, projected, and candidate skill entries |
| `skills[].id` | yes | string | `hc-*` for Helix core IDs, `hr-*` for runtime/projected IDs |
| `skills[].status` | yes | string | `core`, `projected`, or `candidate` |
| `skills[].origin.kind` | yes | string | `helix-core`, `runtime`, or `repo-local` |
| `skills[].origin.source_path` | yes | string | Path to the skill file agents should read when selected |
| `skills[].scope.repos` | yes | list | Repo IDs this skill applies to, empty for global/core skills |
