...
- `helix-repos.yml` — canonical registry manifest name
- `workspaces/<id>/workspace.yml`
- `.helix/install-state.yml`
- `.helix/active-workspace.yml`

> **Legacy:** `repos.yml` was the previous registry name. It is retained as a compatibility alias for older instances. New instances should use `helix-repos.yml`.
...
- workspace setup materializes the active working set under `workspaces/{id}/repos/{repo-id}/`
- `repos[].local_path` semantics remain instance-authored and compatibility-sensitive; existing values should be preserved until a dedicated migration exists
- exactly one workspace is active per CLI session; it is tracked in `.helix/active-workspace.yml`

## `helix-repos.yml`

Purpose: declare the full registry of repos Helix knows how to attach or clone. This is the canonical registry manifest name.
...
| `repos[].local_path` | yes | string | Relative checkout hint/path from the meta-repo root. Keep parity with current runtime behavior; active workspace setup may materialize repos under `workspaces/{workspace-id}/repos/{repo-id}/` without rewriting the registry entry. |
...
> **Compatibility note:** existing instances may keep `local_path` values such as `../orders-api` or `Rapid.Api.PaymentRequest`. Keep those values as-is unless you are intentionally migrating the instance.
...
- full clone metadata already present in `helix-repos.yml`
...
| `repos[].repo_id` | yes | string | Must exist in `helix-repos.yml` |
...
| `artifacts.verification_policy` | no | string | Workspace-relative path to `verification-policy.yml`, if present (beta) |
...
- every `repo_id` must exist in `helix-repos.yml`
...
> **One active workspace per CLI session:** Exactly one workspace is active at any time. Agents read this file to locate the correct workspace artifact tree and active checkout paths.
...
Current setup writes workspace-owned checkout paths here, even when `helix-repos.yml` still carries older compatibility-oriented `local_path` values.
...
|------|----------|------|------|
| `schema_version` | yes | integer | Start with `1` |
| `repo_id` | yes | string | Must match `helix-repos.yml` |
| `local_path` | yes | string | Relative path from the meta-repo root for the currently attached checkout |
...
| `runtime_surface.skill_registry_file` | no | string | Usually `.helix/skills/index.yml` |
...
managed_paths:
  - path: .github/agents/hc-helix.agent.md
    source: .github/agents/hc-helix.agent.md
    category: agent
    sync_mode: replace
    required: true
...
- instance-owned files such as `helix-repos.yml` and `workspaces/*` must not appear here
...
## Beta Runtime Artifact Schemas

The following are **beta** — present when generated or authored, gracefully absent otherwise. See [`helix-process.md`](./helix-process.md#beta-runtime-artifacts) for narrative context.

### `.helix/skills/index.yml`

Purpose: projection ledger of every skill visible at meta-root, used by setup tooling to clean up correctly across workspace switches and by doctor checks to detect drift.

Generated by: `install-helix.ps1` for core `hc-*` skill entries and `setup-workspace.ps1` for the active workspace's projected skills (workspace skills are physically projected to meta-root before the index is written; see [`skill-projection-and-simplification-plan.md`](./skill-projection-and-simplification-plan.md)).

Key fields:

| Field | Required | Type | Notes |
|---|---:|---|---|
| `schema_version` | yes | number | Current value: `2`. v1 indexes are upgraded on read by `Read-HelixSkillIndex` |
| `updated_at` | yes | string | ISO timestamp |
| `active_workspace` | no | string/null | Workspace whose skills were projected when the index was written |
| `skills` | yes | list | Core (`hc-*`), helix-reusable (`hr-*`), and workspace-projected skill entries |
| `skills[].id` | yes | string | `hc-*` for core, `hr-*` for helix-reusable, `{repo-short}-{skill}` for workspace-projected |
| `skills[].status` | yes | string | `core` or `projected` (workspace skills are projected to meta-root, never indexed as raw candidates) |
| `skills[].access.source` | yes | string | `meta-root-skill` for core; `projected` for workspace-projected and `hr-*` |
| `skills[].origin.kind` | yes | string | `helix-core`, `workspace-projected`, or `runtime` |
| `skills[].origin.source_path` | yes | string | Meta-root path to the projected SKILL.md (read this for content) |
| `skills[].projection.from_repo` | conditional | string | Repo short slug; required when `status: projected` and `origin.kind: workspace-projected` |
| `skills[].projection.from_path` | conditional | string | Path inside the source repo of the original SKILL.md folder |
| `skills[].projection.from_name` | conditional | string | Original `name:` frontmatter from the source SKILL.md, before projection rewrote it to the prefixed form |
| `skills[].projection.projected_at` | conditional | string | ISO timestamp when the projection was last written |
| `skills[].projection.checksum` | conditional | string | `sha256:...` over source folder contents; doctor uses this to detect hand-edits or stale projections |
| `skills[].scope.repos` | yes | list | Repo short slug for projected skills; empty for core |
| `skills[].requires_skill_use_record` | yes | boolean | Agents must emit `skill_use` before acting |

Removed in v2 (still tolerated on read for forward migration of v1 files):
- `skills[].routing.use_mode` — every projected skill is invocable from meta-root by definition
- `skills[].access.host_visible` — projection guarantees host visibility

Workspace skills tagged with frontmatter `projection: never` (or `projection: { mode: never }`) are skipped by the projector and do not appear in the index.

### `.helix/repo-capabilities/{repo-id}.yml`

Purpose: cache discovered per-repo capability hints so agents do not re-inspect the repo on every task.

Generated by: `setup-workspace.ps1` during workspace attach/refresh. Onboard may improve the repo guidance that later informs these hints, but does not write the file directly.

| Field | Required | Type | Notes |
|------|----------|------|------|
| `schema_version` | yes | integer | Beta: `1` |
| `repo_id` | yes | string | Must match `helix-repos.yml` |
| `local_path` | yes | string | Active checkout path relative to the meta-repo root |
| `last_scanned_at` | yes | string | ISO timestamp |
| `present` | yes | boolean | Whether the repo checkout was present during the scan |
| `language_hints.primary` | no | string or null | Best-effort primary language hint |
| `language_hints.detected` | yes | list[string] | All discovered language markers |
| `language_hints.confidence` | yes | string | `none`, `low`, `medium`, or `high` |
| `language_hints.notes` | yes | list[string] | Detection caveats |
| `build_hints.systems` | yes | list[string] | Manifest-driven build system hints |
| `build_hints.package_managers` | yes | list[string] | Manifest-driven package manager hints |
| `build_hints.notes` | yes | list[string] | Detection caveats |
| `verification_layers` | yes | list | Abstract verification layers discovered for the repo |
| `verification_layers[].layer` | yes | string | `build`, `unit`, `harness_component`, `contract_sandbox`, `integration_acceptance`, `ui_e2e`, or `release_qualification` |
| `verification_layers[].execution_scope` | yes | string | `local-runnable`, `hybrid`, or `env-backed` |
| `verification_layers[].confidence` | yes | string | `low`, `medium`, or `high` |
| `verification_layers[].evidence` | yes | list[string] | Marker files, directories, or config hints |
| `verification_layers[].trust_notes` | yes | list[string] | Caveats explaining why the hint may be incomplete |

Validation: `repo_id` must exist in `helix-repos.yml`. Do not store readiness state, execution-plan commands, or test results here; use `.helix/repo-state/{repo-id}.yml` for readiness and execution plans for task/slice commands.

### `workspaces/{id}/resume.yml`

Purpose: deterministic resume snapshot read first by the `hc-resume` agent. Captures phase, in-flight tasks, blocked tasks, the last action, and pointers to the canonical artifacts. Distilled memory becomes an optional enrichment rather than the primary state source.

Generated by: `setup-workspace.ps1` on initial workspace setup (with `phase.current = discovery`); patched on subsequent task transitions and decision-log writes via the exported `Update-HelixResumeSnapshot -HelixRoot <path> -WorkspaceId <id> -Patch <hashtable>` function in `helix/scripts/Helix.Tools.psm1`. The function is additive and merge-safe: the existing file is loaded, the patch is deep-merged in, `updated_at` is bumped, and the result is written back.

| Field | Required | Type | Notes |
|------|----------|------|------|
| `schema_version` | yes | integer | Beta: `1` |
| `workspace` | yes | string | Workspace id (must match the parent folder) |
| `updated_at` | yes | string | ISO 8601 UTC; bumped on every `Update-HelixResumeSnapshot` call |
| `phase.current` | yes | string | `discovery`, `design`, `implementation`, `verification`, or `review` |
| `phase.last_completed` | no | string or null | Most recently completed phase from the same enum |
| `current_task` | no | string or null | Task id currently in progress (e.g. `TASK-003`) |
| `last_completed_task` | no | string or null | Most recently completed task id |
| `blocked_tasks` | no | list[string] | Task ids currently in the Blocked section |
| `next_action` | no | string or null | Short imperative the resume agent should propose |
| `artifact_paths.execution_plan` | no | string or null | Workspace-relative path to the execution plan (file or dir) |
| `artifact_paths.task_board` | no | string or null | Workspace-relative path to the task board (file or dir) |
| `artifact_paths.decisions_log` | no | string or null | Workspace-relative path to the decisions log (file or dir) |
| `latest_sessions` | no | list[string] | Recent Copilot CLI session ids; most recent first |
| `verification_debt` | no | list[string] | Short strings describing slices/tasks where verification was deferred |

#### Example

```yaml
schema_version: 1
workspace: order-history
updated_at: 2026-05-05T14:22:00Z
phase:
  current: implementation
  last_completed: design
current_task: TASK-004
last_completed_task: TASK-003
blocked_tasks:
  - TASK-007
next_action: Implement TASK-004 against the OrderHistoryQuery contract.
artifact_paths:
  execution_plan: workspaces/order-history/execution-plans/
  task_board: workspaces/order-history/task-boards/
  decisions_log: workspaces/order-history/decisions/
latest_sessions:
  - 2026-05-05T13-58-21--order-history
verification_debt:
  - "SLICE-2: contract tests deferred pending sandbox creds"
```

#### Validation Rules

- `workspace` MUST match the workspace id (parent folder name).
- `phase.current` MUST be one of the five values listed above.
- `Update-HelixResumeSnapshot` MUST be additive: callers patch only the keys they intend to change; omitted keys are preserved.
- The file is plumbing for `hc-resume`; do not embed narrative content here. Narrative belongs in the task board, decisions log, or distilled memory.

### `workspaces/{id}/verification-policy.yml`

Purpose: operator-authored policy controlling how slice verification gates behave for this workspace.

Optional. Opt in by declaring `artifacts.verification_policy` in `workspace.yml`. When absent, fall back to task-level `commands.verify` plus execution-plan slice verification metadata.

| Field | Required | Type | Notes |
|------|----------|------|------|
| `schema_version` | yes | integer | Beta: `1` |
| `workspace_id` | yes | string | Must match workspace `id` |
| `last_seeded_at` | no | string | ISO timestamp from template seeding |
| `policy.mode` | no | string | `advisory` by default; future runtimes may add stricter modes |
| `policy.trust_model` | no | string | Free-form beta hint such as `beta-heuristic` |
| `policy.default_requirements` | no | list[string] | Verification layers generally expected before a task is treated as locally proven |
| `policy.optional_layers` | no | list[string] | Higher or environment-backed layers that may apply only to some slices |
| `policy.slice_gates.default_gate` | no | string | `run-verify`, `skip`, or `require-human` |
| `policy.slice_gates.on_degraded_confidence` | no | string | `defer-and-record`, `block`, or `skip` |
...
- [`../templates/helix-repos.yml.template`](../templates/helix-repos.yml.template)
- [`../templates/repos.yml.template`](../templates/repos.yml.template) — legacy compatibility alias
